		...
		if( _item.length > 0 && _item[0].STATUS == '1' ) {
			...
			if( InCart ) {
				...
				**/
				...
				if( 
					parseFloat( _item[0].QUANTITE_RESTANTE ) - qte_to_add < 0 
					&& _item[0].TYPE == '1'
					&& _item[0].STOCK_ENABLED == '1'
				) {
					NexoAPI.Notify().warning(
	...
	**/
	...
	this.addToCart 	=	function({ item, barcode, quantity = 1, index = null, increase = true, filter = null}) {
		...
		if( index != null ) {
			...
			currentQty 	=	parseFloat( this.CartItems[ index ].QTE_ADDED );
		}
		...
		**/
		...
		if( item.STATUS == '1' ) {
			...
			let remainingQuantity 	=	parseFloat( $( gridItem ).attr( 'data-remaining-quantity' ) );
			...
			if( increase && index ) {
				testQuantity 		=	parseFloat( v2Checkout.CartItems[ index ].QTE_ADDED ) + quantity;
			}
		...
		if( _.toArray( this.CartItems ).length > 0 ){
			...
			_.each( this.CartItems, function( value, key ) {
				...
				$( '#cart-table-body' ).find( 'table' ).append(
					'<tr cart-item-id="' + key + '" cart-item data-line-weight="' + ( MainPrice * parseFloat( value.QTE_ADDED ) ) + '" data-item-barcode="' + value.CODEBAR + '">' +
	...
	this.checkItemsStock			=	function( items ) {
		...
		if( isEnabled == 'yes' ) {
			...
			_.each( items, function( value, key ) {
				var leftPercentage	=	( parseFloat( value.QUANTITE_RESTANTE ) * 100 ) / parseInt( value.QUANTITY );
