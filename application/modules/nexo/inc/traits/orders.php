        ...
        foreach ( $this->post( 'ITEMS' ) as $item) {
            ...
            **/
			...
			if( intval( $item[ 'stock_enabled' ] ) == '1' && $item[ 'inline' ] != '1' ) {
                ...
                $data                       =   [];
                $data[ 'QUANTITE_VENDU' ]   =   floatval( $fresh_item[0][ 'QUANTITE_VENDU' ]) + floatval($item[ 'qte_added' ]);
                ...
                if( in_array( intval( $fresh_item[0][ 'TYPE' ] ), $this->events->apply_filters( 'treat_as_sold_quantity', [ 1 ] ) ) ) {
                    $data[ 'QUANTITE_RESTANTE' ]   =   floatval($fresh_item[0][ 'QUANTITE_RESTANTE' ]) - floatval($item[ 'qte_added' ]);
                }
        ...
        **/
        ...
        foreach ( $old_order[ 'products' ] as $product ) {
            if( intval( $product[ 'STOCK_ENABLED' ] ) == 1 ) {
                ...
                if( floatval( $fresh_product[0][ 'QUANTITE_RESTANTE' ] ) - floatval($product[ 'QUANTITE' ]) > 0 ) {
                    $this->db
			...
			**/
			...
			if( intval( $item['stock_enabled'] ) == 1 && $item[ 'inline' ] != '1' ) {
				$this->db->where('CODEBAR', $item['codebar'])->update( store_prefix() . 'nexo_articles', array(
					'QUANTITE_RESTANTE'        =>    floatval($fresh_item[0][ 'QUANTITE_RESTANTE' ]) - floatval($item['qte_added']),
					'QUANTITE_VENDU'        =>    floatval($fresh_item[0][ 'QUANTITE_VENDU' ]) + floatval($item['qte_added'])
				) );
			}
	...
	public function order_refund_post( $order_code )
	{
        ...
        foreach( $itemToRemove as $orderItemID => $quantity ) {
            ...
            ->update( store_prefix() . 'nexo_commandes_produits', [
                'QUANTITE'      =>  floatval( $orderItem[0][ 'QUANTITE' ] ) - $quantity
            ]);
